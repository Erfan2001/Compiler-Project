...
def dfa(str1: str , lineNumber : int ):
    ...
    i = 0
    while(i < len(str1)):
        if(not finder(re.findall("^[a-zA-Z_0-9]*", str1[i]))):
            token = str1[begin:i]
            if(finder(re.findall("^[0-9]+[a-zA-Z]+", token))):
            ...
            if(str1[i] in comparisonArray):
                currentIndex = i
                while (str1[i] in comparisonArray):
                    i += 1
                if(len(finder(re.findall(comparisonRegex, str1[currentIndex:i]))) == 1):
                    output.append(("comparison", str1[currentIndex:i]))
                else:
                    print("Line %d has lexical Error (comparison)" % index)
            if(str1[i] in operationArray):
                currentIndex = i
                while (str1[i] in operationArray):
                    i += 1
                if(len(finder(re.findall(operationRegex, str1[currentIndex:i]))) == 1):
                    output.append(("operation", str1[currentIndex:i]))
                else:
                    print("Line %d has lexical Error (operation)" % index)
            begin = i+1
        i += 1
